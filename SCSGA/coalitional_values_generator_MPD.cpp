...
void coalitional_values_generator_MPD::reset
(
	uint32_t n_agents,
	uint32_t n_tasks,
	int seed
)
{
	...
	error_generator = std::normal_distribution<coalition::value_t>(0.0f, 0.1f);
	...
	// TODO Try to reincorporate this with the new coalitional values generation.
...
coalition::value_t coalitional_values_generator_MPD::generate_new_value(
	const coalition::coalition_t& coalition, const uint32_t task
)
{
	...
	coalition::value_t error_value = error_generator(generator);

	coalitional_value = skill_value + relation_value + error_value;
	return coalitional_value;
}

std::string coalitional_values_generator_MPD::get_file_name() const
{
	return "MPD_" + std::to_string(seed) + "_" + std::to_string(n_agents) + "_" + std::to_string(n_tasks) + ".problem";
}

coalitional_values_generator_MPD::coalitional_values_generator_MPD() :
